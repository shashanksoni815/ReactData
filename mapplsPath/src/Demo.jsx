...
const MapplsDirectionApp = () => {
  ...
  // Replace with your Security Key (REST API Key) from Mappls Dashboard
  const MAPPLS_REST_KEY = 'horvnxetwyihqjwvflvrypsdaavmuogjeatj';
  ...
  useEffect(() => {
    ...
    style.href = `https://apis.mappls.com/advancedmaps/api/${MAPPLS_REST_KEY}/map_sdk?layer=vector&v=3.0`;
    ...
    try {
      ...
      if (result.latitude && result.longitude) {
        ...
        if (geocodeData.copResults && geocodeData.copResults.length > 0) {
          destLat = parseFloat(geocodeData.copResults[0].latitude || geocodeData.copResults[0].lat);
          destLng = parseFloat(geocodeData.copResults[0].longitude || geocodeData.copResults[0].lng);
        }
      }
      ...
      if (routeData.routes && routeData.routes.length > 0) {
        ...
        // Draw new route
        if (route.geometry && window.mappls.Polyline) {
                  ...
                  >
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-sm text-gray-500">{result.placeAddress}</div>
                        {result.eLoc && (
              ...
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                {routeInfo.trafficDelay > 0 && (
                  <div className="text-xs text-orange-600 mt-1">
                    +{routeInfo.trafficDelay} min due to traffic
                  </div>
                )}
              </div>
        ...
        {loading && (
          <div className="absolute inset-0 bg-black bg-opacity-30 flex items-center justify-center z-10">
            <div className="bg-white p-6 rounded-lg shadow-xl flex items-center gap-3">
              ...
              <span className="text-gray-700 font-medium">Finding best route with traffic...</span>
            </div>
          </div>
        )}
        ...
        {!mapLoaded && (
          <div className="absolute inset-0 bg-gray-100 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500 mt-2">Make sure your API key is valid</p>
            </div>
          </div>
        )}
      </div>
      ...
      {!routeInfo && mapLoaded && !loading && (
        <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 bg-white p-4 rounded-lg shadow-xl max-w-md z-10">
          <p className="text-sm text-gray-600 text-center">
            <strong>🔍 Search for any destination</strong> to view route, travel time, and real-time traffic conditions
          </p>
        </div>
      )}
    </div>
